...
# CUDA_VISIBLE_DEVICES=0 python main.py --config configs/officehome.yaml
# CUDA_VISIBLE_DEVICES=0 python main.py --config configs/mydataset.yaml
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            for idx in range(image_features.shape[0]):
                train_features.append(image_features[idx].detach().cpu().numpy())
                train_cache_keys.append(cache_keys[idx].detach().cpu().numpy())
            
            l = target.cpu().numpy().tolist()
            labels.extend(l)
            ## <<<
            cache_logits = ((-1) * (beta - beta * affinity)).exp() @ cache_values
        ...
        if plot_tsne == True:
            ...
            train_features_tsne = TSNE(n_components=2, init='pca', random_state=42).fit_transform(train_features)

            train_cache_keys = np.array(train_cache_keys)
            train_cache_keys = train_cache_keys.reshape(train_cache_keys.shape[0], -1)
            train_cache_keys_tsne = TSNE(n_components=2, init='pca', random_state=42).fit_transform(train_cache_keys)

            # Plot the t-SNE visualization
            fig = plt.figure(figsize=(12, 12))
            plt.subplot(111)
            plt.title('2D-TSNE epochs {}'.format(train_idx))
            
            unique_elements, counts = np.unique(labels, return_counts=True)
            print("check elements: ", np.asarray((unique_elements, counts)).T)

            for label in np.unique(labels):
                # use labels to find index
                print("now label: ", label)
                print("len:", len(labels == label))
                if label == 0:
                    c = 'b'
                    l = 'normal'
                else:
                    ...
                    l = 'defect'
                print("l: ", l)
                plt.scatter(train_features[labels == label, 0], train_features[labels == label, 1], label=l, s=5, c=c)
            plt.legend()
            plt.savefig("./tsne_img/features_{}.png".format(train_idx))
            ...
            for label in np.unique(labels):
                ...
                if label == 0:
                    ...
                    c = 'r'
                    l = 'defect'
                plt.scatter(train_cache_keys_tsne[labels == label, 0], train_cache_keys_tsne[labels == label, 1], label=l, s=5, c=c)
            plt.legend()
            plt.savefig("./tsne_img/cache_keys_{}.png".format(train_idx))
